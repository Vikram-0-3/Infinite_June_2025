...
class Program
    {
        static void Main()
        {
            ...
            {
                con = new SqlConnection("Data Source=ICS-LT-5CD244D\\SQLEXPRESS ;Initial Catalog=InfiniteDB;" + "user id = sa;password = 63744724052906");
                con.Open();
                return con;
            }
            ...
            {
                SqlCommand cmd = new SqlCommand("sp_getavgsal_empcount", con);
                cmd.CommandType = CommandType.StoredProcedure;

                // Add input parameter
                cmd.Parameters.AddWithValue("@did", 10); // example department ID

                // Add output parameter
                SqlParameter avgParam = new SqlParameter("@avgsal", SqlDbType.Float)
                {
                    Direction = ParameterDirection.Output
                };
                cmd.Parameters.Add(avgParam);
                ...
                float avgSalary = Convert.ToSingle(avgParam.Value);
                int empCount = Convert.ToInt32(returnParam.Value);
